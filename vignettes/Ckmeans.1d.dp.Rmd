---
title: "Tutorial: Optimal univariate $k$-means clustering"
author: "Joe Song and Haizhou Wang"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tutorial: Optimal univariate k-means clustering}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This tutorial illustrates applications of optimal univariate $k$-means clustering function ```Ckmeans.1d.dp```. It clusters univariate data given the number of clusters $k$. It can estimate $k$ if not provided. It can also perform optimal weighted clustering when a weight vector is provided with the input univariate data. Weighted clustering can be used to analyze 1-D signals such as time series data. The corresponding clusters obtained from weighted clustering can be the basis for optimal time course segmentation or optimal peak calling. 
...
We segment a time course to identify peaks using weighted $k$-means. The input data is the time stamp of obtaining each intensity measurement; the weight is the signal intensity.
