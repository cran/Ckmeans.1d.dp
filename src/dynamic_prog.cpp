   ...
   */
{
  ...
  std::vector<ldouble> sum_w(w.size());
  ...
  for(int q = 1; q < K; ++q) {
    ...
    if(method == "linear") {
      fill_row_q_SMAWK(imin, N-1, q, S, J, sum_x, sum_x_sq, sum_w);
    } else if(method == "loglinear") {
      fill_row_q_log_linear(imin, N-1, q, q, N-1, S, J, sum_x, sum_x_sq, sum_w);
    } else if(method == "quadratic") {
      fill_row_q(imin, N-1, q, S, J, sum_x, sum_x_sq, sum_w);
    } else {
    ...
    fill_row_q_log_linear(imin, N-1, q, q, N-1, SS, JJ, sum_x, sum_x_sq, sum_w);
